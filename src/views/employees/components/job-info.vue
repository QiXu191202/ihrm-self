...
import { getEmployeeListApi, getJobInfoApi, updateJob } from "@/api/employees";
...
export default {
  ...
  methods: {
    async initData() {
      let res = await getJobInfoApi(this.userId);
      let { rows } = await getEmployeeListApi({ page: 1, size: 1000 });
      this.formData = res;
      this.depts = rows;
    },
    async saveJob() {
